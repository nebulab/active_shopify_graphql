...
module ActiveShopifyGraphQL
  ...
  class Relation
    ...
    # @param of [Integer] Records per page (default: 250, max: 250)
    ...
    def first(count = nil)
      if count
        spawn(total_limit: count, per_page: [count, MAX_PER_PAGE].min).to_a
      else
    ...
    # Build a QueryScope for backward compatibility with PaginatedResult
      ...
      connection_names.each do |name|
        if name.is_a?(Hash)
          ...
          name.each do |key, nested_value|
            ...
            nested_relation = Relation.new(target_class)
