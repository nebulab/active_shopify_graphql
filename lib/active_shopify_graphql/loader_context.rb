...
module ActiveShopifyGraphQL
  ...
  class LoaderContext
    attr_reader :graphql_type, :loader_class, :defined_attributes, :model_class, :included_connections, :fields

    def initialize(graphql_type:, loader_class:, defined_attributes:, model_class:, included_connections: [], fields: nil)
    ...
    def for_model(new_model_class, new_graphql_type: nil, new_attributes: nil, new_connections: [])
      model_fields = new_model_class.respond_to?(:fields) ? new_model_class.fields : nil
    ...
    def is_metaobject?
      model_class.is_a?(Class) && model_class <= ActiveShopifyGraphQL::Metaobject
    end

    def metaobject_type
      return nil unless is_metaobject?

      model_class.metaobject_type
    end
  end
end
