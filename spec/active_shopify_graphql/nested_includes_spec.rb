...
RSpec.describe "Nested Includes" do
  ...
  it "generates correct GraphQL query for nested includes" do
    ...
    # We can inspect the fragment generated by the loader
    fragment = ActiveShopifyGraphQL::FragmentBuilder.new(loader).build_connection_fragments
    expect(fragment).to include("lineItems(first: 50) {")
    expect(fragment).to include("edges {")
    expect(fragment).to include("node {")
    expect(fragment).to include("variant {")
    expect(fragment).to include("product {")
  end
  ...
  it "extracts nested data correctly" do
    ...
    connection_data = ActiveShopifyGraphQL::ResponseMapper.new(loader).extract_connection_data(response_data)
